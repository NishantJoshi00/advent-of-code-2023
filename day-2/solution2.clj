...
(defn max-reduce [a1 a2]
  (map #(max (get %1 0) (get %1 1)) (map vector a1 a2)))
...
(defn read-input []
  (try
    (let [[game data] (str/split (read-line) #": ")]
      [1, (->Game (idx-game game) (reduce * (reduce max-reduce (get-max-cubes data))))])
    (catch Exception _ [0])))
...
(defn input-loop [my-map]
  (let [[state data] (read-input)]
    (if (= state 0)
      my-map
      (recur (+ my-map (:state data))))))

(defn main []
  ; (println (read-input)) (println (read-input)))
  (println (input-loop 0)))
(main)
