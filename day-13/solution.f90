...
subroutine solve(hwrite, vwrite, input, size)
  ...
  logical, dimension(len(trim(input(1)))-1) :: happlicative
  logical, dimension(size - 1) :: vapplicative
  logical :: rev_check
  ...
  do i = 1,mylen-1
    do j = 1, size
      mlen = MIN(i, mylen - i)
      happlicative(i) = rev_check(input(j)(i-mlen+1:i), input(j)(i+1:i+mlen), mlen) .and. happlicative(i)
    end do
  end do
  ...
  do i = 1,size-1
    do j = 1, mylen
      mlen = MIN(i, size - i)
      vapplicative(i) = rev_check(inputT(j)(i-mlen+1:i), inputT(j)(i+1:i+mlen), mlen) .and. vapplicative(i)
    end do
  end do


  hwrite = FINDLOC(vapplicative, .true., DIM=1)

  vwrite = FINDLOC(happlicative, .true., DIM=1)
